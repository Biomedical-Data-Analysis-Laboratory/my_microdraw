...
var dbroot = "https://"+localhost+"/php/microdraw_db.php";
...
function appendRegionTagsFromOntology(o) {
    ...
    for( var i = 0; i < o.length; i++ ) {
        if( o[i].parts ) {
            $("#regionPicker").append("<div><b>"+o[i].name+"</b></div>");
...
*/
	...
	// added felix 171018 to solve error when dealing with touch screens
...
function copyAnnotationFromAutoTo(user_to_copy_to) {
    ...
    */
        ...
        if (ImageInfo[sl]["needToSave"] == false) {
            ...
            var slice = ImageInfo[sl]; //ImageInfo[sl];
            var value = {};
            value.Regions = [];
            for( var i = 0; i < slice.Regions.length; i++ )
            {
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log("< microdrawDBSave resolve: Successfully copied regions: ",ImageInfo[sl].Regions.length," slice: " + sl.toString(), " to user: " +  user_to_copy_to, " response:",data);
...
function loadImage(name) {
    ...
    // F170710: to remove drawing tools in case of guest user or add them otherwise
    checkGuestUser(); // better done here than in microdrawDBLoad() (executes later)
...
function initMicrodraw() {
    ...
    if(window.location.href.indexOf("images") > -1)
    {
      ...
      appendRegionTagsFromOntology(NMIBC);
    }
    //añadido el 03/11/ 2020
...
function highlight(item){
    console.log("item", selected.classList)
	if (selected){
...
function initMicrodraw2(obj) {
	...
	for( var i = 0; i < obj.tileSources.length; i++ ) {
		...
		var imagePath = obj.tileSources[i];
                imagePathParts = imagePath.split("/")[imagePath.split("/").length-1].split("_"); //just until the first underscore. Before: (imagePath.split("/"));

                var node=document.createElement("li");
        	var textnode=document.createTextNode(imagePathParts[0] + '_' + imagePathParts[1] + '_' + imagePathParts[2]);
        	node.appendChild(textnode);
        	document.getElementById("ImageList").appendChild(node);
	}
	...
	for( var i = 0; i < imageOrder.length; i++ ) {
        	...
        	ul.addEventListener('click',function(e){
            	clicked_idx = e.target.index
            	highlight(e.target);
            	console.log(clicked_idx);
            	update_slider_value(clicked_idx);
            	loadImage(imageOrder[clicked_idx]);
